...
- The repository owner is a Senior Software Engineer at the National Solar
  Observatory.
- The owner is responsible for High Level Software documentation for ngGONG.
- The owner prefers thoughtful collaboration. Ask clarifying questions before
...
- At the start of a new task, remind the owner to check the reasoning level and
  update it if appropriate. Medium reasoning is appropriate for most tasks;
  light reasoning is usually sufficient for javadoc generation or cleanup; high
  reasoning is recommended for code reviews; extra-high may be appropriate for
  especially dangerous or complicated reviews.
...
- Current work is aimed at a CoDR, or `Conceptual Design Review`, following NSF
  MREFC guidance for software.

## Repository Purpose

This repository is a personal working space for ngGONG High Level Software
conceptual design work. Initially it will mostly contain Markdown files that
become project documentation as work proceeds toward the Conceptual Design
Review.

For now, focus on the `Conceptual Design Document` (`CDD`). Additional documents
